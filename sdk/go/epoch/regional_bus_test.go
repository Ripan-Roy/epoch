...
func TestRegionalBusClientRoutesCompleteMutationAndReadContracts(t *testing.T) {
	...
	if len(operations) != 11 {
		t.Fatalf("expected 11 operations, got %d", len(operations))
	}
	if operations[0].Path != base+"/mutations" || operations[7].Path != base+"/mutations/12" {
	...
	if operations[8].Path != base+"/archive/replay" || operations[9].Path != base+"/deliveries/query" {
		t.Fatalf("unexpected Bus read paths: %#v", operations[8:])
	}
	for _, request := range operations[7:] {
