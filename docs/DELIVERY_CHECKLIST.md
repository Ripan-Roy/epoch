...
| G6 | Compatibility gateways | 🟡 | Named protocol/client matrix, differential tests, fuzzing, malformed frames, migration evidence | The bounded Redis/Kafka/AMQP gateway, exact client matrix, native adapter contracts, scanner, and docs passed protected PR #102/main CI. The next feature candidate adds atomic Redis conditional set/get, lossless Kafka timestamps/ordered duplicate headers, rejection-aware acknowledgements, and a passing local production-image regional recovery campaign (PC-08). Protected combined-campaign evidence, differential/fuzz certification, richer Redis types, Kafka membership/transactions, broader AMQP routing/1.0, MQTT, and comparative performance remain open. |
...
| PC-08 | Prove real-client compatibility | CI | 🟡 | The combined production-image campaign pins Redis CLI 8.8.2, Kafka Java 4.3.1, and RabbitMQ Java 5.34.0. Local evidence passes binary Cache/TTL/counters and atomic conditional set/get; all four Kafka codecs, CreateTime, ordered duplicate/null headers and checkpoints; AMQP confirms/requeue/expired leases/ack permanence and capacity refusal; gateway replacement, each profile leader loss, all-voter SIGKILL/reopen, and replica convergence. Redis non-string overwrite, timestamp/header loss, and false native-rejection confirmations were reproduced red and fixed. CI reuses exact candidate images and uploads `epoch.protocol-regional.evidence/v1`; protected branch/main execution remains. |
