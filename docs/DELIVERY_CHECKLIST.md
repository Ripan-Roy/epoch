...
**Current core target:** Replicated Stream time, size, and combined retention
...
| ID | Checklist item | Boundary | State | Evidence / acceptance |
|---|---|---|---:|---|
| RT-01 | Freeze deterministic policy, byte, time, and compatibility semantics | Stream + ADR | 🟡 | ADR-0023 fixes complete policy replacement, compact canonical JSON byte accounting, inclusive expiry, monotonic committed time, hard bounds, and unchanged v1/v2/v3 command bytes; protected merge remains pending |
| RT-02 | Enforce time, size, count, and combined deletion atomically | Rust Stream core | 🟡 | Focused tests cover exact age boundary, oldest-first combined enforcement, oversized-record rollback, dedupe removal, disabled bounds, and snapshot validation; full regression remains pending |
| RT-03 | Replicate retention through a versioned command | Rust tablet | 🟡 | Canonical v4 configure/maintain mutations return typed receipts, replay exactly, converge digests, reject kind/version drift, and preserve golden v1/v2/v3 behavior locally |
...
| RT-05 | Expose direct and regional operations in every first-party SDK | HTTP + Go/Java/Python | 🟡 | Strict configure, maintain, and linearizable observe routes plus all three repository-local clients and executable quickstarts pass focused contract tests |
| RT-06 | Recover the exact retained boundary | Rust node + checkpoints | 🟡 | A real three-voter test configures age retention, commits maintenance, converges every voter, checkpoints, reopens all EPRS paths, and restores base/end/policy/watermark; the regional Python SDK container campaign also passes leader loss and same-volume recovery locally |
| RT-07 | Publish truthful product and operator documentation | Docs + Pages | 🟡 | Stream/API/semantics/architecture/testing/plan/traceability/changelog/SDK/ADR and docs-page content describe exact semantics and non-claims; format, content assertions, and the docs production bundle pass locally, while main-only publication remains pending |
| RT-08 | Pass full local and protected evidence | Quality + GitHub | 🟡 | `make check`, `make build`, the three-process partition/`SIGKILL` smoke, executable quickstarts, and regional Docker all pass; PR checks, merge SHA, exact-main CI, and Pages evidence remain pending |
