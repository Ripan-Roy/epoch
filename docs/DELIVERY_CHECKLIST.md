...
**Current release target:** `v0.2.0-beta.7`
**Current core target:** Beta hardening through bounded production telemetry and operator-integrated latency diagnosis
...
| G6 | Compatibility gateways | 🟡 | Named protocol/client matrix, differential tests, fuzzing, malformed frames, migration evidence | The bounded Redis/Kafka/AMQP gateway, exact client matrix, native adapter contracts, scanner, and docs passed protected PR #102/main CI. The current candidate locally proves atomic structured Redis collections, replicated classic Kafka groups/claims/fenced commits, AMQP fanout/topic routing, TTL and mandatory returns, plus compatible dependency automation through exact production images, released clients, gateway replacement, leader loss, and all-voter reopen. Protected combined-campaign evidence, differential/fuzz certification, remaining Redis commands, Kafka static/cooperative/new-group protocols and transactions, durable AMQP topology/header routing/DLX/1.0, MQTT, and comparative performance remain open. |
...
| G10 | Release readiness | 🟡 | Synchronized versions, CI, Pages, notes, verified tag provenance, artifacts, security and compatibility statements | PR #102 corrected the five-component native-build/finalize matrix and consolidated dependency/security updates; exact-main CI `33987709595` and Pages `33987709501` passed at `635f98f`. Beta.6 remains untagged while the real-regional campaign's timestamp/header and false-confirmation fixes complete protected review. Exact-tag publication and independent manifest/signature/SBOM verification, raw signed binaries, deferred package-manager artifacts, and GA operating evidence remain open. |
...
| PC-03 | Support high-value Redis client traffic | RESP2/RESP3 | 🟡 | HELLO/AUTH/client setup, binary-safe values, atomically fenced NX/XX/GET plus EX/PX, non-mutating `WRONGTYPE`, MGET/MSET, delete/existence/type, signed counters, TTL/expiry/persist, hashes, lists, sets, sorted sets, RESP3, and pipelining pass semantic/parser tests and Redis CLI 8.8.2. Structured mutations preserve TTL/storage class under native version fences. Transactions/scripts, Pub/Sub/Streams, binary collection members/keys, cluster/TLS, and broader differential conformance remain open. |
| PC-04 | Support Kafka producer and consumer traffic | Kafka wire | 🟡 | ApiVersions advertises only dispatched Produce 3–9, Fetch 4–12, ListOffsets 1–7, Metadata 1–12, FindCoordinator 0–4, JoinGroup 0–9, SyncGroup 0–5, Heartbeat 0–4, LeaveGroup 0–5, OffsetCommit 2–9, and OffsetFetch 1–7. gzip/Snappy/LZ4/Zstd batches and Kafka Java 4.3.1 producer/manual/classic-group consumer flows pass the exact production-image fault campaign; replicated sessions assign arbitrary shard counts, Sync installs claims, and commits are member/generation fenced. Static/multi-topic/cooperative/new-group membership, idempotence/transactions/admin/SASL and protected combined conformance remain open. |
| PC-05 | Support RabbitMQ work delivery lifecycle | AMQP 0-9-1 | 🟡 | PLAIN/open/tune/heartbeat, channels, existing Queue declaration, process-shared direct/fanout/topic bindings, bind/unbind/delete, binary publish, native TTL, string headers, mandatory returns, confirms, push consume/basic.get, QoS, ack/reject/nack/requeue, cancel, and routing metadata pass tests plus the RabbitMQ Java 5.35.0 production-image fault campaign. Durable topology, header exchanges, arguments/policies, DLX configuration, transactions, AMQP 1.0/TLS and protected combined conformance remain open. |
...
| PC-08 | Prove real-client compatibility | CI | 🟡 | The combined production-image campaign pins Redis CLI 8.8.2, Kafka Java 4.3.1, and RabbitMQ Java 5.35.0. The current campaign locally passes binary Cache/TTL/counters, atomic conditional set/get and structured collections; all four Kafka codecs, CreateTime, ordered duplicate/null headers, classic consumer groups and checkpoints; AMQP topic routing, TTL, mandatory returns, confirms/requeue/expired leases/ack permanence and capacity refusal; gateway replacement, each profile leader loss, all-voter SIGKILL/reopen, and replica convergence. CI reuses exact candidate images and uploads `epoch.protocol-regional.evidence/v1`; protected branch/main execution remains for this candidate. |
...
| 3 | Synchronize Rust, Go, Java, Python, TypeScript, SDK user agents, and lockfiles | `./scripts/check-release-version.sh` passes locally at `0.2.0-beta.7`; protected verification remains | 🟡 |
| 4 | Write curated, version-controlled release notes | `docs/releases/v0.2.0-beta.7.md` names behavior, artifacts, upgrade guidance, verification, observability boundaries, and beta limitations; merge remains | 🟡 |
| 5 | Pass protected `main` CI and main-only Pages | Both workflow runs green at the same commit | ⬜ |
| 6 | Verify the live docs show the beta.7 release, observability guide, SDK content, and **Why slow?** workflow | Public Pages bundle assertions | ⬜ |
...
| 9 | Publish the GitHub release from the checked-in notes | GitHub prerelease targets `v0.2.0-beta.7` | ⬜ |
...
After `v0.2.0-beta.7` completes this sequence, the table above intentionally
