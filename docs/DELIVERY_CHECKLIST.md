...
**Current core target:** Leader-owned signed webhook delivery
...
| G4 | Native profile cores | 🟡 | Cache, Stream, Queue, and Bus P0 semantics with truthful public routing and fault evidence | All four typed tablet cores run simultaneously behind resource/shard routing and pass process/container recovery; all four have authenticated versioned three-language clients. Signed HTTP/webhook delivery now passes a real three-process retry/reopen proof locally; Queue/Stream, unsigned target execution, and remaining P0 breadth are open |
| G5 | Trust and observability | 🟡 | Identity, authorization, TLS/mTLS, encryption, audit, telemetry, quotas, explain | Shared Go/Rust bootstrap authentication, scoped action authorization, collection isolation, session-only console credential, and credential-free decision logs pass protected `main` CI. Regional topology now exposes local maintenance counters and the container campaign asserts zero errors locally; OIDC, expiry/revocation, TLS/mTLS/peer identity, encryption, replicated policy, immutable audit export, exported telemetry/alerts, and quotas remain open |
...
| G7 | Data services and integrations | 🟡 | Schemas, pipes, connectors, target execution, checkpoints, transaction boundaries | Bus intent/outbox and a public-HTTPS-only signed webhook executor pass locally; Queue/Stream targets, private egress, schemas, and connectors remain open |
...
| MT-13 | Real-process and container fault campaign | Test infrastructure | MT-06–MT-12 | 🟡 | Three policy-protected regional containers cover topology/capacity, catalog plus seven tablets/four profiles including a three-shard Stream, authenticated Go recovery, and separate active-leader losses followed by real Python Stream sessions plus Stream, Queue, Cache, and Event Bus lifecycles. The campaign proves all five maintenance classes without client-triggered sweeps, automatic checkpoint/compaction on all 24 voter/group copies, session-fenced consumption, voter catch-up, all-node `SIGKILL`, same-volume reopen, and digest convergence in protected CI through PR #68; broader crash/I/O/auth-abuse faults remain |
| MT-14 | Documentation, traceability, changelog, and release notes | Cross-cutting | MT-13 | 🟡 | ADR-0010–0029, architecture, security, APIs, testing, development, SDK/runtime guides, traceability, checklist, published docs content, and `Unreleased` notes describe the implemented boundaries and non-claims through merged PR #68; final version notes and release remain a later release cut |
...
| ID | Checklist item | Boundary | State | Evidence / acceptance |
|---|---|---|---:|---|
| WH-01 | Freeze leader ownership, delivery order, crypto, replay, and non-claims | ADR + semantics | 🟡 | ADR-0030 defines exact lease-before-I/O, at-least-once recovery, HMAC-SHA-256 input, `(delivery ID, attempt)` replay identity, key overlap rotation, response classes, and external-side-effect limits; protected merge remains pending |
| WH-02 | Preserve existing command and snapshot bytes | Rust core | 🟡 | Unsigned histories remain v1; signed targets, exact internal acquire, and terminal reject use v2. Golden, wrong-version, legacy snapshot, and deterministic candidate tests pass locally |
| WH-03 | Dispatch only from the current replicated owner | Rust regional runtime | 🟡 | The current non-fail-stopped Event Bus leader commits and awaits an exact candidate lease, performs I/O outside the state machine, then commits Ack/retry/reject with deterministic proposal identity; Clippy and focused suites pass locally |
| WH-04 | Enforce HTTPS, SSRF, DNS, redirect, proxy, timeout, and metadata boundaries | Rust security | 🟡 | HTTPS/public-only validation, explicit loopback development, per-attempt all-address DNS validation/pinning, IANA special-address corpus including IPv4-in-IPv6, no redirects/proxies, strict headers, discarded bodies, a complete DNS-plus-request lease cap with no expired-lease request, and bounded redacted key files pass locally; network egress proxy and penetration review remain open |
| WH-05 | Align target creation and verification in all first-party SDKs | Go + Java + Python | 🟡 | Signed target constructors serialize the key ID; exact-body verifiers share one vector, constant-time comparison, canonical decimal/lowercase-hex parsing, timestamp tolerance, and replay identity. Full Go/Java/Python suites pass locally |
| WH-06 | Prove real retry, convergence, and recovery | Real-process integration | 🟡 | A real loopback receiver returns 503 then 204; attempts 1/2 have the same delivery ID and distinct signatures. Every voter converges the failed/Ack history and returns it after all three processes reopen existing storage |
| WH-07 | Publish end-to-end receiver and operator documentation | Docs + Pages | 🟡 | Regional Bus guide, API/security/semantics/runtime docs, SDK READMEs, ADR, traceability, and the rendered docs page cover key setup, signed target creation, raw-body verification, replay storage, outcomes, and non-claims; the production docs build and content assertions pass locally, while main-only Pages publication remains pending |
| WH-08 | Pass full local and protected evidence | Quality + GitHub | 🟡 | Complete `make check`, all-language build, real-process suite, Docker integration campaign, and displayed SDK quickstarts pass locally; feature PR CI, squash merge, exact-main CI, and main-only Pages verification remain pending |
