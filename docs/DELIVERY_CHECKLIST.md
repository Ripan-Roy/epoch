...
| G3 | Consensus, catalog, and placement | 🟡 | Quorum safety, persistent catalog, multi-group supervision, membership, placement, repair, read barriers | Fixed voters and deterministic catalog pass; catalog consensus and runtime multiplexing are next |
| G4 | Native profile cores | 🟡 | Cache, Stream, Queue, and Bus P0 semantics with truthful public routing and fault evidence | All four typed tablet cores pass internally; public multi-tablet routing and remaining P0 breadth are open |
...
| G8 | Managed operations | ⬜ | Durable Go reconciliation, operator, autoscaling, backup, metering, billing, private networking | Go desired-state scaffold exists; it is not connected to Rust regional authority |
...
| MT-06 | Persist catalog commands through a dedicated consensus group | Rust regional control | G2, G3, MT-04 | ⬜ | Three-voter commit, minority rejection, restart, corruption, and catch-up tests |
| MT-07 | Demultiplex peer frames by group and epoch | Rust node transport | MT-06 | ⬜ | Unknown group/epoch rejection, isolation, bounded backpressure, and ordering tests |
| MT-08 | Supervise several consensus groups in one node process | Rust node runtime | MT-07 | ⬜ | Independent failure, shutdown/drain, resource caps, and recovery tests |
| MT-09 | Materialize typed profile tablets from committed catalog state | Rust regional control | MT-06, MT-08 | ⬜ | Create/update/delete/restart convergence across all four profiles |
| MT-10 | Route public requests by resource, shard, leader, generation, and epoch | Rust gateway | MT-09 | ⬜ | Stale route, not-leader, fenced generation, retry, and isolation tests |
| MT-11 | Reconcile hosted desired state through the Rust authority | Go control plane | MT-06, MT-10 | ⬜ | gRPC lifecycle, idempotency, disconnect, conflict, and observed-generation tests |
| MT-12 | Show achieved placement and risk without overclaiming | TypeScript console | MT-10, MT-11 | ⬜ | Resource health, empty placement, degraded state, and accessibility tests |
| MT-13 | Real-process and container fault campaign | Test infrastructure | MT-06–MT-12 | ⬜ | Multiple groups/resources, leader loss, one/all-node `SIGKILL`, reopen, and convergence |
| MT-14 | Documentation, traceability, changelog, and release notes | Cross-cutting | MT-13 | ⬜ | Docs build, exact examples, requirement links, next version notes |
