...
| G5 | Trust and observability | 🟡 | Identity, authorization, TLS/mTLS, encryption, audit, telemetry, quotas, explain | Shared Go/Rust authorization and audit pass protected `main`. The current candidate adds internal Prometheus endpoints, bounded tenant series, structured correlation, W3C propagation, OTLP traces, a measured “Why slow?” path, and tested dashboard/alert/collector templates across data, control, compatibility, operator, and console boundaries. OIDC, expiry/revocation, rotation automation, replicated policy, immutable audit export, exhaustive profile state gauges, support bundles, production telemetry/SLO evidence, and quotas remain open. |
| G6 | Compatibility gateways | 🟡 | Named protocol/client matrix, differential tests, fuzzing, malformed frames, migration evidence | The bounded Redis/Kafka/AMQP gateway, exact client matrix, native adapter contracts, scanner, and docs passed protected PR #102/main CI. The current candidate additionally passes focused, exact released-client, and production-image regional-v2 tests for atomic `MSET`/`MSETNX`, bounded Kafka static identity rejoin, AMQP headers routing, provisioned native DLX forwarding, leader loss, and all-voter reopen. Protected evidence, differential/fuzz certification, remaining Redis commands, full static/cooperative/new-group Kafka protocols and transactions, durable AMQP topology/named-DLX/1.0, MQTT, and comparative performance remain open. |
...
| G10 | Release readiness | 🟡 | Synchronized versions, CI, Pages, notes, verified tag provenance, artifacts, security and compatibility statements | `v0.2.0-beta.7` is an annotated exact-main tag at `177fd17`; CI `34260192724`, main-only Pages `34260192695`, and release-tag workflow `34282006344` passed. The GitHub prerelease, five signed multi-platform OCI manifests, and ten digest-bearing SPDX assets are public. Raw signed binaries, deferred package-manager artifacts, independent clean-cluster digest-pull evidence, and GA operating evidence remain open. |
...
## Current observability delivery

| ID | Checklist item | Boundary | State | Evidence / acceptance |
|---|---|---|---:|---|
| OB-01 | Bound every metric dimension | Rust + Go | 🟡 | Closed profile/operation/outcome/stage/protocol labels, deterministic tenant fingerprints, 1–4,096 configurable admission, and explicit overflow series pass unit/contract tests; protected CI remains. |
| OB-02 | Correlate requests without leaking data | Rust + Go | 🟡 | Safe `X-Request-ID`, JSON-compatible structured completion events, trace IDs, status, and duration omit tokens, payloads, resource names, and raw tenant labels; broader log-redaction corpus remains open. |
| OB-03 | Propagate and export traces | Rust + Go | 🟡 | W3C extraction/injection crosses public HTTP, Go gRPC/regional HTTP, and Redis/Kafka/AMQP-to-native HTTP; validated optional OTLP/HTTP exporters batch and flush with bounded shutdown; protected collector evidence remains. |
| OB-04 | Attribute tail latency from measurements | Data + control + console | 🟡 | Regional routing/replication/storage stage samples feed an internal diagnostic, exact tenant authorization, bounded failover, strict browser decoding, and “no measured cause” state; additional quota/target instrumentation remains. |
| OB-05 | Ship operator-ready templates | Deployment | 🟡 | Internal Kubernetes Services and Compose ports, one memory-limited Collector config, six-panel Grafana dashboard, and actionable Prometheus rules are parsed and policy-checked in CI; live monitoring-stack campaign remains. |
| OB-06 | Publish end-to-end operating docs | Documentation | 🟡 | `OBSERVABILITY.md`, ADR-0045, and the GitHub Pages observability guide cover ports, PromQL, OTLP, SDK propagation, “Why slow?”, deployment, runbooks, cardinality, and non-claims; main-only Pages publication remains. |
| OB-07 | Complete every PRD 14.1/14.2 operational signal | Profile runtimes | ⬜ | Add bounded profile state gauges, consumer inspection/rebalance history, “Can I lose data?”, replay/redrive preview, and redacted support-bundle automation with fault/load evidence. |
...
| ID | Checklist item | Boundary | State | Evidence / acceptance |
|---|---|---|---:|---|
| PC-01 | Keep protocol I/O outside replicated engines | Rust gateway | 🟡 | `epoch-compat` depends on one narrow async semantic port and translates into authenticated, generation/tablet/term-fenced regional Cache, Stream, and Queue calls. The real-image campaign passes gateway replacement, profile-leader losses, and full-voter reopen locally; protected execution remains. |
| PC-02 | Bound adversarial protocol input and concurrency | Rust gateway | 🟡 | 8 MiB frames, 4 MiB bodies/cumulative Kafka expansion, 1,024 logical items, preflighted Kafka record counts, bounded Zstandard windows, bounded per-listener connection semaphores, strict trailing-byte rejection, AMQP state ordering, RESP partial-frame handling, and deterministic RESP/Kafka-record/AMQP mutation corpora pass local tests. Coverage-guided fuzzing plus minimized crash-corpus retention remains open. |
| PC-03 | Support high-value Redis client traffic | RESP2/RESP3 | 🟡 | HELLO/AUTH/client setup, binary-safe values, atomically fenced NX/XX/GET plus EX/PX, non-mutating `WRONGTYPE`, bounded atomic `MSET`/`MSETNX`, MGET, delete/existence/type, signed counters, TTL/expiry/persist, hashes, lists, sets, sorted sets, RESP3, and pipelining pass semantic/native-adapter tests and Redis CLI 8.8.2. Structured mutations preserve TTL/storage class; multi-set uses one revision-fenced transaction and never partially succeeds. Transactions/scripts, Pub/Sub/Streams, binary collection members/keys, cluster/TLS, and broader differential conformance remain open. |
| PC-04 | Support Kafka producer and consumer traffic | Kafka wire | 🟡 | ApiVersions advertises only dispatched Produce 3–9, Fetch 4–12, ListOffsets 1–7, Metadata 1–12, FindCoordinator 0–4, JoinGroup 0–9, SyncGroup 0–5, Heartbeat 0–4, LeaveGroup 0–5, OffsetCommit 2–9, and OffsetFetch 1–7. gzip/Snappy/LZ4/Zstd batches and Kafka Java 4.3.1 producer/manual/classic-group flows pass; replicated sessions assign arbitrary shard counts, Sync installs claims, and commits are member/generation fenced. Bounded static identities reuse their native member and fence instance mismatches across group APIs. Simultaneous duplicate-owner fencing, multi-topic/cooperative/new-group membership, idempotence/transactions/admin/SASL and protected combined conformance remain open. |
| PC-05 | Support RabbitMQ work delivery lifecycle | AMQP 0-9-1 | 🟡 | PLAIN/open/tune/heartbeat, channels, existing Queue declaration, process-shared direct/fanout/topic/headers bindings, bind/unbind/delete, binary publish, native TTL, string headers, mandatory returns, confirms, push consume/basic.get, QoS, ack/reject/nack/requeue, cancel, and routing metadata pass tests and RabbitMQ Java 5.35.0. Headers support bounded `x-match=all|any`; default-exchange DLX arguments must match a provisioned native target, and durable forwarding removes expiration and adds first-death metadata. Durable topology, named-DLX routing, general arguments/policies, `x-death`, transactions, AMQP 1.0/TLS and protected combined conformance remain open. |
...
| PC-08 | Prove real-client compatibility | CI | 🟡 | The combined production-image campaign pins Redis CLI 8.8.2, Kafka Java 4.3.1, and RabbitMQ Java 5.35.0. Evidence v2 covers binary Cache/TTL/counters, atomic conditional and multi-set operations and structured collections; all four Kafka codecs, CreateTime, ordered duplicate/null headers, classic groups, static identity rejoin and checkpoints; AMQP topic/headers routing, TTL/DLX, mandatory returns, confirms/requeue/expired leases/ack permanence and capacity refusal; gateway replacement, each profile leader loss, all-voter SIGKILL/reopen, and replica convergence. Exact-client fixture and production-image regional-v2 conformance are green locally; protected execution remains for this candidate. |
| PC-09 | Scan migrations before cutover | CLI | 🟡 | `epoch-compat scan` emits bounded `epoch.compatibility-scan/v1` text/JSON assessments with source lines, CI failure thresholds, supported/partial/unknown/unsupported outcomes, Kafka version ranges and static/cooperative boundaries, Redis option/database boundaries including `MSETNX`, and an unsupported feature corpus. Dynamic traffic capture and protected evidence remain open. |
...
| OCI-04 | Publish only an exact current-main tag | GitHub Actions + GHCR | ✅ | The only publication trigger is pushed `v*`; synchronized version, checked-in notes, exact current `main`, and successful CI/Pages push runs at that SHA are required. `v0.2.0-beta.7` passed those gates at `177fd17`, and the bounded finalize stage published only the exact tag after receiving one amd64 and one arm64 digest per component. There is no `latest` or manual publication path. |
| OCI-05 | Bind provenance and SBOMs to immutable digests | GitHub Actions | ✅ | BuildKit and GitHub provenance target the assembled beta.7 manifests and their platform digests. Ten digest-bearing SPDX release assets cover the five amd64/arm64 component pairs, and the protected tag workflow verified the attestations before release publication. |
| OCI-06 | Sign and verify every manifest | Sigstore + GitHub | ✅ | Pinned Cosign keylessly signed all five beta.7 manifests and verified their exact repository workflow/tag identity and GitHub OIDC issuer; `gh attestation verify` also gated the successful protected release-tag workflow `34282006344`. |
| OCI-07 | Publish consumer verification and non-claims | Docs + Pages | ✅ | ADR-0041, the release-artifact guide, security, operator, releasing, PRD, traceability, README, changelog, and the public deployment page define digest-first verification, architecture/SBOM scope, the no-`latest` policy, and deferred packages/raw binaries. Main-only Pages `34260192695` published the beta.7 release and verification guidance. |
| OCI-08 | Prove clean pull and protected release | GitHub + Kubernetes | 🟡 | The beta.7 annotated tag, protected workflow, curated prerelease, five public amd64/arm64 OCI indexes, signatures/attestations, and ten SBOM assets are verified. A separate clean Kubernetes environment must still pull the published digests, reverify them independently, and run the release lifecycle before this gate is complete. |
...
|---:|---|---|---:|
| 1 | Select a completed, merged milestone boundary | PR #120 merged bounded metrics, trace propagation, latency diagnosis, operator/deployment assets, SDK validation, dependency refresh, docs, and traceability after the complete protected matrix | ✅ |
| 2 | Choose the next semantic prerelease version | `v0.2.0-beta.7` follows the published beta.6 release and does not already exist | ✅ |
| 3 | Synchronize Rust, Go, Java, Python, TypeScript, SDK user agents, and lockfiles | Protected release verification passed at `0.2.0-beta.7` | ✅ |
| 4 | Write curated, version-controlled release notes | `docs/releases/v0.2.0-beta.7.md` is merged and is the published prerelease body | ✅ |
| 5 | Pass protected `main` CI and main-only Pages | CI `34260192724` and Pages `34260192695` passed at `177fd17` | ✅ |
| 6 | Verify the live docs show the beta.7 release, observability guide, SDK content, and **Why slow?** workflow | The public Pages bundle exposes beta.7, protocol documentation, and **Why slow?** | ✅ |
| 7 | Create an annotated tag at the exact current `main` commit | Annotated `v0.2.0-beta.7` resolves to `177fd17`, the then-current main commit | ✅ |
| 8 | Pass tag/version/main provenance workflow | Release-tag workflow `34282006344` passed | ✅ |
| 9 | Publish the GitHub release from the checked-in notes | The `v0.2.0-beta.7` GitHub prerelease was published on 8 September 2026 | ✅ |
| 10 | Verify downloads and package claims | Five public OCI indexes and ten digest-bearing platform SBOM assets exist; package-manager publication remains explicitly deferred | ✅ |
| 11 | Start the next `Unreleased` section | Changelog prepared for continued delivery | ✅ |

This table records the completed `v0.2.0-beta.7` sequence. The next feature
release starts from `Unreleased` and must repeat every step with new exact-main
evidence.
