...
| HA-09 | Prove local cross-language behavior | Go + Rust tests | 🟡 | Repository-wide lint and unit/race suites, patched dependency audit, all-language build, all rendered configs, standalone/docs smoke, and the full regional Docker ownership/failover/all-voter recovery campaign pass locally. A regression reproduces the former 7.5 MiB periodic-control checkpoint and proves the compact image plus durable replay stay below 4 MiB. Protected PR evidence and dedicated concurrent multi-control chaos remain. |
