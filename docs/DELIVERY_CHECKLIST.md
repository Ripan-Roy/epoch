...
| G4 | Native profile cores | 🟡 | Cache, Stream, Queue, and Bus P0 semantics with truthful public routing and fault evidence | All four typed tablet cores run simultaneously behind resource/shard routing and pass protected process/container recovery; all four have authenticated versioned three-language clients. PR #74 and exact-main CI `32365193683` prove signed HTTP/webhook retry and reopen; Queue/Stream, unsigned target execution, and remaining P0 breadth are open |
...
| G7 | Data services and integrations | 🟡 | Schemas, pipes, connectors, target execution, checkpoints, transaction boundaries | Bus intent/outbox and a public-HTTPS-only signed webhook executor pass protected `main`; Queue/Stream targets, private egress, schemas, and connectors remain open |
...
| G10 | Release readiness | 🟡 | Synchronized versions, CI, Pages, notes, verified tag provenance, artifacts, security and compatibility statements | `v0.1.0-alpha.4` is the current source-prerelease target; packaged artifacts and GA evidence remain open |
...
| MT-14 | Documentation, traceability, changelog, and release notes | Cross-cutting | MT-13 | 🟡 | ADR-0010–0030, architecture, security, APIs, testing, development, SDK/runtime guides, traceability, checklist, published docs content, and `v0.1.0-alpha.4` notes describe the implemented boundaries and non-claims through merged PR #74; tag provenance and GitHub prerelease publication are the remaining release-cut actions |
