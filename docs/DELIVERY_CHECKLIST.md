...
| PC-08 | Prove real-client compatibility | CI | 🟡 | The combined production-image campaign pins Redis CLI 8.8.2, Kafka Java 4.3.1, and RabbitMQ Java 5.34.0. Local evidence passes binary Cache/TTL/counters; all four Kafka codecs, CreateTime, ordered duplicate/null headers and checkpoints; AMQP confirms/requeue/expired leases/ack permanence and capacity refusal; gateway replacement, each profile leader loss, all-voter SIGKILL/reopen, and replica convergence. Timestamp/header loss and false native-rejection confirmations were reproduced red and fixed. CI reuses exact candidate images and uploads `epoch.protocol-regional.evidence/v1`; protected branch/main execution remains. |
