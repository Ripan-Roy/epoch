...
**Current release target:** `v0.1.0-alpha.9`
**Current core target:** Complete Event Bus and integration development (`BUS-001`–`BUS-015`)
...
| G4 | Native profile cores | 🟡 | Cache, Stream, Queue, and Bus P0 semantics with truthful public routing and fault evidence | All four typed tablet cores run simultaneously behind resource/shard routing and authenticated three-language clients. Cache, Stream, and Queue completion shipped in alpha.6–alpha.8. The alpha.9 candidate closes the native Event Bus development surface; production protocol/scale evidence remains open. |
...
| G7 | Data services and integrations | 🟡 | Schemas, pipes, connectors, target execution, checkpoints, transaction boundaries | The alpha.9 candidate adds replicated schemas/validation, transforms/enrichment, MQTT state, catalogs/endpoints, connector checkpoints/replay, and leader-owned API/function/managed-target execution to the shipped signed and Epoch targets. External source polling, private egress, official protocol conformance, and production connector certification remain open. |
...
| G10 | Release readiness | 🟡 | Synchronized versions, CI, Pages, notes, verified tag provenance, artifacts, security and compatibility statements | `v0.1.0-alpha.8` is the current published source prerelease; `v0.1.0-alpha.9` is the Event integration feature-release target. The repository is MIT licensed; packaged artifacts, signing/SBOM provenance, and GA evidence remain open. |
...
| MT-13 | Real-process and container fault campaign | Test infrastructure | MT-06–MT-12 | 🟡 | Three policy-protected regional containers cover topology/capacity, catalog plus simultaneous four-profile tablets, authenticated Go recovery, and separate active-leader losses followed by real Python SDK lifecycles. Stream state services, session fencing, signed-webhook and Queue/Stream target delivery, Cache completion, advanced Queue leader loss/session/dedupe/correlation/defer/DLQ forwarding, voter catch-up, all-node `SIGKILL`, same-volume reopen, and digest convergence pass protected `main` through alpha.8. The alpha.9 candidate adds managed structured-CloudEvent delivery, stable idempotency, settlement convergence, and full Event integration state reopen. Broader crash/I/O/auth-abuse evidence remains open. |
| MT-14 | Documentation, traceability, changelog, and release notes | Cross-cutting | MT-13 | 🟡 | ADR-0010–0036, executable SDK guides, release notes, and main-only Pages are published through `v0.1.0-alpha.8`; exact-main Pages run `32514143435` is green. ADR-0037 and the Event integration documentation pass local source/build checks for alpha.9; protected publication remains open. |
...
| ID | Checklist item | Boundary | State | Evidence / acceptance |
|---|---|---|---:|---|
| EI-01 | Freeze state, delivery ordering, security, compatibility, and non-claims | ADR + contracts | 🟡 | ADR-0037 defines replicated ownership, leader lease-before-I/O, checkpoint-before-settlement, snapshot admission/restore validation, safe egress, stable idempotency, compatibility, and explicit alpha limits; protected review remains. |
| EI-02 | Complete routing, filters, transforms, rate limits, retention, and redrive | Rust Bus/tablet | 🟡 | Wildcard/header/JSON filters, projection/rename/constants/templates/enrichment, integer rate/burst, DLQ retention/redrive, archive count/age retention, and bounded long poll pass focused engine/tablet tests locally. |
| EI-03 | Replicate schemas, MQTT state, catalog, endpoints, functions, and connectors | Rust Bus/tablet | 🟡 | Strict bounded registries, monotonic revisions, schema policies, MQTT sessions/retained/QoS/shared cursors, endpoint health, catalog lineage, function state, connector partial outcomes/checkpoints/replay/secret versions, canonical snapshots, and semantic restore validation pass locally. |
| EI-04 | Execute managed HTTP and connector targets safely | Rust node | 🟡 | Source-leader exact lease, binary/structured CloudEvents, API-key/OAuth client credentials for API destinations, optional bearer credentials for function/connector calls, a strict secret file, allowlists, public-only DNS pinning, disabled redirect/proxy, token cache, endpoint failure observation, and committed connector checkpoint ordering pass seven focused worker tests including a real OAuth/target receiver. |
| EI-05 | Persist Bus configuration and expose strict regional APIs | Rust catalog/node | 🟡 | Catalog canonicalizes the complete Bus capacity/archive/outbox/retention contract, materialization consumes it on every voter, integration state and operations are fenced/authenticated, and topology reports the managed executor. Focused catalog/materializer/topology tests pass locally. |
| EI-06 | Expose the complete regional lifecycle in every SDK | Go + Java + Python | 🟡 | All three SDKs expose rate limits, retention/redrive/maintenance, long poll, managed target/auth/CloudEvents models, and generic strict integration operations with route/fence/idempotency tests. Typed builders for every integration-operation variant remain a usability improvement, not a transport gap. |
| EI-07 | Prove real failover, side effects, convergence, and reopen | Real three-process runtime | 🟡 | The expanded process campaign delivers a structured CloudEvent with a stable idempotency key, commits Ack on all voters, stops/reopens all voters, and proves Ack plus full state convergence. The complete Docker regional campaign also passes locally across simultaneous Stream, Queue, Cache, and Bus tablets, leader replacement, voter catch-up, all-node reopen, and Go/Java/Python forced-restart quickstarts; protected execution remains. |
| EI-08 | Bound every live and recovery state path | Rust Bus/tablet | 🟡 | Integration operations are clone-staged and atomically rejected beyond 2 MiB; every successful tablet mutation must remain Event-Bus-snapshot encodable; restore revalidates identities, revisions, bounds, references, receipts, checkpoints, URLs, MQTT state, canonical bytes, and digests. Focused capacity/corruption tests pass locally. |
| EI-09 | Publish end-to-end documentation and honest feature coverage | Docs + console + Pages | 🟡 | PRD note, ADR-0037, traceability/checklist, regional SDK/API/runtime/security/testing guides, console documentation, executable snippets, alpha.9 notes, and local Pages bundle assertions pass locally. Protected main-only publication remains. |
| EI-10 | Pass one large feature PR and release alpha.9 | Quality + GitHub | 🟡 | Exact-tree `make check`, `make build`, and `make test-integration` pass locally across Rust, Go, Java, Python, console, lint, docs, native-process, container, recovery, and displayed quickstart gates. One protected feature PR, exact-main CI, main-only Pages, annotated tag provenance, and the curated GitHub prerelease remain. |
...
| 2 | Choose the next semantic prerelease version | `v0.1.0-alpha.9` is later than alpha.8 and does not already exist | ✅ |
| 3 | Synchronize Rust, Go, Java, Python, TypeScript, SDK user agents, and lockfiles | `./scripts/check-release-version.sh` passes at `0.1.0-alpha.9` | ✅ |
| 4 | Write curated, version-controlled release notes | `docs/releases/v0.1.0-alpha.9.md` names highlights, verification, compatibility, limitations, and source-only artifacts | ✅ |
...
| 9 | Publish the GitHub release from the checked-in notes | GitHub prerelease targets `v0.1.0-alpha.9` | ⬜ |
...
After `v0.1.0-alpha.9` completes this sequence as a source-only prerelease, the
