...
**Current core target:** Profile-native snapshot/restore and bounded physical storage reclamation
...
| G2 | Storage and recovery | 🟡 | Checksummed formats, crash recovery, corruption policy, snapshots, compaction, retention, tiering | Segmented WAL, compatible EPSN v1/v2 checkpoints, native images for all five profiles, checkpoint-plus-tail reopen, fixed-voter catch-up, and physical EPRS reclamation pass the complete local cross-language and container gates; protected merge, retention, tiering, backup/PITR, and restore campaigns remain open |
...
| MT-13 | Real-process and container fault campaign | Test infrastructure | MT-06–MT-12 | 🟡 | Three policy-protected regional containers cover topology/capacity, catalog plus five resources/four profiles, authenticated Go recovery, and separate active-leader losses followed by real Python Stream, Queue, Cache, and Event Bus lifecycles, voter catch-up, all-node `SIGKILL`, same-volume reopen, and digest convergence locally; this increment's protected-branch CI and broader crash/I/O/auth-abuse faults remain |
| MT-14 | Documentation, traceability, changelog, and release notes | Cross-cutting | MT-13 | 🟡 | ADR-0010–0020, architecture, security, APIs, testing, development, SDK/runtime guides, traceability, checklist, published docs content, and `Unreleased` notes describe the implemented boundaries and non-claims; final version notes and release follow merge and green `main` |
...
| ID | Checklist item | Boundary | State | Evidence / acceptance |
|---|---|---|---:|---|
| NC-01 | Freeze compatible bytes, bounds, ordering, and non-claims | Format + ADR | 🟡 | ADR-0022, EPSN v2, and additive EPRS kind 4 specify a 4 MiB profile limit, 6 MiB image limit, 1,024-record/1 MiB retry suffix, final frame digest, durable order, and backup/PITR non-claims; protected merge remains pending |
| NC-02 | Preserve EPSN v1 and legacy EPRS histories | Rust consensus | 🟡 | V1 fixtures remain unchanged; v1 reopen, explicit v1-to-v2 conversion, and the complete all-features workspace regression pass locally |
| NC-03 | Capture and install every native profile canonically | Rust profiles + tablets | 🟡 | Catalog, Stream, Queue, Cache, and Event Bus validate complete scope/configuration, canonical bytes, state/payload digests, corruption, unknown version, and domain invariants in the green all-features suite |
| NC-04 | Bound retry metadata without weakening live state | Rust consensus + profiles | 🟡 | The newest exact-retry suffix survives checkpoint/reopen, aged-out IDs become `unknown`, and each profile retains matching typed receipts independently of complete live business state; consensus/profile regressions pass locally |
...
| NC-06 | Restore profiles before applying retained tails | Rust node + containers | 🟡 | The profile-neutral lagging-voter test installs a Catalog image before its tail; every profile's real three-voter restart forces a checkpoint and restores before readiness; 112 node tests plus regional and four profile-specific container campaigns pass |
| NC-07 | Publish architecture, operations, format, traceability, and docs-page content | Docs + Pages | 🟡 | Repository docs and the rendered docs page describe v2 behavior/non-claims; formatting, lint, typecheck, content assertions, 8 tests, and the docs-only `/epoch/` production bundle pass locally; main-only Pages publication remains pending |
| NC-08 | Pass full local and protected pull-request evidence | Quality + GitHub | 🟡 | Rust fmt/clippy/docs/audit/all-features tests/build, Go race tests/build, Python lint/type/test/wheel, Java verify/JAR, generated contracts, workflow/shell lint, SDK smokes, multiprocess crash, and all container campaigns pass locally; PR checks, merge SHA, exact-main CI, and Pages remain pending |
