...
**Current release target:** `v0.1.0-alpha.8`
**Current core target:** Complete Queue state services (`QUEUE-007`–`QUEUE-015`)
...
| G10 | Release readiness | 🟡 | Synchronized versions, CI, Pages, notes, verified tag provenance, artifacts, security and compatibility statements | `v0.1.0-alpha.7` is the current published source prerelease; `v0.1.0-alpha.8` is the Queue feature-release target. The repository is MIT licensed; packaged artifacts, signing/SBOM provenance, and GA evidence remain open. |
...
| MT-13 | Real-process and container fault campaign | Test infrastructure | MT-06–MT-12 | 🟡 | Three policy-protected regional containers cover topology/capacity, catalog plus simultaneous four-profile tablets, authenticated Go recovery, and separate active-leader losses followed by real Python SDK lifecycles. Stream state services, session fencing, signed-webhook and Queue/Stream target delivery, Cache completion, voter catch-up, all-node `SIGKILL`, same-volume reopen, and digest convergence pass protected `main` through alpha.7. The alpha.8 candidate adds an advanced Queue leader loss, session/dedupe/correlation/defer/DLQ forwarding, catch-up, and full reopen; the rebuilt local campaign passes. Broader crash/I/O/auth-abuse evidence remains open. |
| MT-14 | Documentation, traceability, changelog, and release notes | Cross-cutting | MT-13 | 🟡 | ADR-0010–0035, executable SDK guides, release notes, and main-only Pages are published through `v0.1.0-alpha.7`; exact-main Pages run `32485430871` is green. ADR-0036 and the Queue completion documentation pass local source/build checks for alpha.8; protected publication remains open. |
...
| ID | Checklist item | Boundary | State | Evidence / acceptance |
|---|---|---|---:|---|
| QS-01 | Freeze capacity, ordering, dispatch, expiry, and forwarding semantics | ADR + contracts | 🟡 | ADR-0036 defines exact admission, dedupe ordering, FIFO lock fences, priority aging, token bucket/breaker, temporary destinations, durable target binding, crash ordering, compatibility, and non-claims; protected review is pending. |
| QS-02 | Preserve historical Queue contracts | Rust Queue/tablet | 🟡 | Existing command v1/v2, snapshot v1 reads, legacy recovery checksum golden, and result encodings pass locally; advanced operations use v3 and snapshot v2. |
| QS-03 | Implement bounded capacity, expiry, dedupe, and fairness | Rust Queue | 🟡 | Count/byte admission, three overflow policies, non-lease victim choice, durable idle expiry, dedupe-before-overflow, and committed-time priority aging pass focused deterministic tests. |
| QS-04 | Implement sessions, dispatch protection, defer, and correlation | Rust Queue/tablet | 🟡 | Exclusive renewable session locks, FIFO selection, rate/burst/concurrency, circuit transitions, exact defer/receive, correlation metadata, full digest, and snapshot recovery pass locally. |
| QS-05 | Deliver dead letters crash-safely | Rust node | 🟡 | Quorum-only catalog admission, pending/bound/completed outbox, source-leader ownership, exact incarnation binding, stable target key, target-before-complete ordering, and unit recovery/key tests pass locally. |
| QS-06 | Persist configuration and expose strict regional APIs | Rust catalog/node | 🟡 | Catalog normalization and materialization retain advanced configuration; strict v3 request conversion and linearizable advanced/correlation/outbox reads pass node tests. |
| QS-07 | Expose the complete lifecycle in every SDK | Go + Java + Python | 🟡 | All SDK suites pass session metadata/acquire/renew/release, defer/exact receive, advanced state, correlation, and outbox route/auth/fence contracts. |
| QS-08 | Prove failover, forwarding, convergence, and reopen | Compose | 🟡 | The rebuilt campaign kills the advanced Queue leader, runs the Python session/dedupe/correlation/defer lifecycle, observes the forwarded target DLQ record, catches up the stopped voter, kills every node, reopens the same volumes, and proves converged state. Local execution passes; protected execution remains. |
| QS-09 | Publish end-to-end documentation | Docs + Pages | 🟡 | Queue guide/tablet/ADR, exact three-language quickstarts, PRD/traceability/checklist, API/semantics/testing docs, and public Pages content are staged locally. |
| QS-10 | Pass feature PR, exact-main gates, and release alpha.8 | Quality + GitHub | 🟡 | Exact-tree `make check`, `make build`, and `make test-integration` pass locally, including strict all-target lint/Rustdoc, every language suite, dependency audit, all Docker fault campaigns, and executable documentation. Protected PR, exact-main CI, main-only Pages, tag provenance, and the GitHub release remain. |
...
| 2 | Choose the next semantic prerelease version | `v0.1.0-alpha.8` is later than alpha.7 and does not already exist | ✅ |
| 3 | Synchronize Rust, Go, Java, Python, TypeScript, SDK user agents, and lockfiles | `./scripts/check-release-version.sh` passes at `0.1.0-alpha.8` | ✅ |
| 4 | Write curated, version-controlled release notes | `docs/releases/v0.1.0-alpha.8.md` names highlights, verification, compatibility, limitations, and source-only artifacts | ✅ |
...
| 9 | Publish the GitHub release from the checked-in notes | GitHub prerelease targets `v0.1.0-alpha.8` | ⬜ |
...
After `v0.1.0-alpha.8` completes this sequence as a source-only prerelease, the
