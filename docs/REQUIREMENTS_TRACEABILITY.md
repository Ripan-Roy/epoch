...
| DX-004 | P0 | Test containers and ephemeral namespaces | M1 → M2 | Slice | G1, G5, G10 | A unique three-node Compose project uses independent volumes, dynamic loopback ports, and mounted policy; it proves Go recovery plus simultaneous four-profile failover/catch-up/all-node recovery. Post-failover Python runs Stream, Queue, complete Cache, and Event Bus lifecycles. Protected campaigns retry a signed receiver, deliver to Queue/keyed Stream without duplicates, exercise complete Cache and Stream state services, and reopen every voter. The alpha.8 local campaign adds Queue sessions, defer/correlation, capacity, and crash-safe DLQ forwarding. Pending: protected alpha.8 evidence, parallel campaigns, Go/Java live regional execution, broader isolation, and injected disk/network matrices |
