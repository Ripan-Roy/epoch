...
| ID | Pri | Capability shorthand | Milestone | Status | Dependency gates | Verification evidence placeholder |
|---|---:|---|---|---|---|---|
| DX-001 | P0 | Official Go, Java, and Python SDKs | M1 one SDK → M2 | Slice | G0, G1, G4, G10 | Go/Java/Python standalone tests and exact-source restart quickstarts remain green. All three implement shared regional Stream and complete Queue discovery/auth/fence/idempotent-retry/read-barrier contracts with unit tests and checked-in Pages examples; Python executes both profiles after real leader loss. Pending: generated regional response types, coordinated streaming sessions, Cache/Event-Bus regional clients, package publication, and the complete native contract/version matrix |
| DX-002 | P0 | Generated guarantee-aware API docs | M1 → M2 | Slice | G0, G1, G10 | Hand-authored guarantee/error guidance, exact executable standalone quickstarts, and exact compilable regional Stream/Queue Go/Java/Python sources are built into the docs-only Pages artifact; ADR-0017/0018 and both regional SDK guides own route/retry/non-claim semantics. Pending: generated API reference and full doc lint |
...
| DX-004 | P0 | Test containers and ephemeral namespaces | M1 → M2 | Slice | G1, G5, G10 | A unique three-node Compose project uses independent volumes, dynamic loopback ports, and mounted policy; it proves Go recovery plus simultaneous four-profile failover/catch-up/all-node recovery. Post-failover Python Stream performs append/retry/fetch/checkpoint/lag; post-failover Python Queue performs enqueue replay, credit/lease lifecycle, DLQ/redrive, linearizable reads, 11-command convergence, catch-up, and reopen. Pending: parallel campaigns, Go/Java live regional execution, broader isolation, and injected disk/network matrices |
