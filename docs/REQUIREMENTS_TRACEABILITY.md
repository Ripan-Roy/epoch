...
| BUS-004 | P0 | Per-target retry, timeout, rate, DLQ | M4 | Slice | G0, G2, G4, G5 | Captured per-subscription timeout/max-in-flight/retry policy, deterministic backoff/jitter, pure earliest in-flight deadline, bounded lease-expiry maintenance, attempt exhaustion, dead-letter state, fencing, and authenticated regional SDK validation pass. The current regional leader now proposes due lease timeout automatically; Python proves pending retry/reacquire after Event Bus leader loss without fail/maintenance calls, then voter convergence and EPRS reopen. Pending: rate limiting, redrive/retention, executor crash matrix, and external audit |
...
| ID | Pri | Capability shorthand | Milestone | Status | Dependency gates | Verification evidence placeholder |
|---|---:|---|---|---|---|---|
| DX-001 | P0 | Official Go, Java, and Python SDKs | M1 one SDK → M2 | Slice | G0, G1, G4, G10 | Go/Java/Python standalone tests and exact-source restart quickstarts remain green. All three share regional Stream, complete Queue, complete Cache, and complete implemented Event Bus discovery/auth/fence/idempotent-retry/read-barrier contracts with unit tests and Pages examples. Stream clients additionally expose identical UTF-8 shard selection, generation-pinned keyed append, atomic caller-framed batches, session lifecycle, low-level per-shard claim/fenced fetch, and a resource-generation-pinned claim–revalidate helper; Python executes three Stream shards and all four profiles after real leader loss. The claim slice passed PR #68 and exact-main CI `31726157672`. Pending: generated regional response types, persistent native streaming/cooperative revoke, package publication, and the complete native contract/version matrix |
| DX-002 | P0 | Generated guarantee-aware API docs | M1 → M2 | Slice | G0, G1, G10 | Hand-authored guarantee/error guidance, exact executable standalone quickstarts, and exact compilable regional Stream/Queue/Cache/Event Bus Go/Java/Python sources are built into the docs-only Pages artifact; ADR-0017–0020 plus ADR-0023–0029 and the regional SDK guides own route/retry/non-claim semantics. Stream examples include keyed routing, gzip batch, checkpoints, sessions, claim–revalidate, fenced fetch, and retention. Main-only Pages run `31726157684` published the claim slice and live-bundle verification passed. Pending: generated API reference and full doc lint |
