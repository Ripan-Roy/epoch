...
partial G10 evidence only: signed binaries, SBOM/provenance,
...
| DX-004 | P0 | Test containers and ephemeral namespaces | M1 → M2 | Slice | G1, G5, G10 | A unique three-node Compose project uses independent volumes, dynamic loopback ports, and mounted policy; it proves Go recovery plus simultaneous four-profile failover/catch-up/all-node recovery. Post-failover Python runs Stream, Queue, complete Cache, and Event Bus lifecycles. The protected real-process campaign retries a signed receiver and delivers to Queue/keyed Stream, then reopens every voter without duplicate target records. The expanded Cache campaign adds byte/cold state, advanced queries, backup/PITR, changes, Pub/Sub, and multiplex; protected execution is pending. Pending: parallel campaigns, Go/Java live regional execution, broader isolation, and injected disk/network matrices |
