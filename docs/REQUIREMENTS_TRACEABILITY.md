...
| CACHE-002 | P0 | Key/default TTL and expiry events | M1 → M2 | Slice | G0, G1, G2, G4 | Deterministic expiry, pure passive reads, committed bounded maintenance, time rollback, failover, replay, three-language TTL/maintain methods, and current-leader regional value/lock expiry are tested. The Python campaign observes TTL reclamation without a maintenance call after Cache leader loss and through recovery; pending: expiry events, eviction coupling, scale/SLO evidence, and broader fault matrix |
...
| ID | Pri | Capability shorthand | Milestone | Status | Dependency gates | Verification evidence placeholder |
|---|---:|---|---|---|---|---|
| DX-001 | P0 | Official Go, Java, and Python SDKs | M1 one SDK → M2 | Slice | G0, G1, G4, G10 | Go/Java/Python standalone tests and exact-source restart quickstarts remain green. All three share regional Stream, complete Queue, complete Cache, and implemented Event Bus discovery/auth/fence/retry/read-barrier contracts. Signed target constructors and constant-time exact-body webhook verifiers share the same canonical vector, timestamp window, and replay identity. Stream clients additionally expose identical UTF-8 shard selection, generation-pinned keyed append, atomic caller-framed batches, session lifecycle, low-level per-shard claim/fenced fetch, and a resource-generation-pinned claim–revalidate helper; Python executes three Stream shards and all four profiles after real leader loss. Exact-main CI `32365193683` covers every language and the cross-language smoke. Pending: generated regional response types, persistent native streaming/cooperative revoke, package publication, and the complete native contract/version matrix |
| DX-002 | P0 | Generated guarantee-aware API docs | M1 → M2 | Slice | G0, G1, G10 | Hand-authored guarantee/error guidance, exact executable standalone quickstarts, and exact compilable regional Stream/Queue/Cache/Event Bus Go/Java/Python sources are built into the docs-only Pages artifact. ADR-0030 covers signed receivers; ADR-0031 and the current Bus guide/examples cover Queue/Stream provisioning, target construction, automatic execution, read-only binding evidence, recovery, and cross-tablet non-claims. Main-only publication of the current update remains pending. Pending: generated API reference and full doc lint |
...
| DX-004 | P0 | Test containers and ephemeral namespaces | M1 → M2 | Slice | G1, G5, G10 | A unique three-node Compose project uses independent volumes, dynamic loopback ports, and mounted policy; it proves Go recovery plus simultaneous four-profile failover/catch-up/all-node recovery. Post-failover Python runs Stream, Queue, Cache, and Event Bus lifecycles. The real-process campaign additionally retries a signed receiver and automatically delivers to Queue and a keyed multi-shard Stream across differing group leaders, then reopens every voter without duplicate target records. Protected evidence for the Queue/Stream addition remains pending. Pending: parallel campaigns, Go/Java live regional execution, broader isolation, and injected disk/network matrices |
