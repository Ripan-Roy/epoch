...
| STREAM-002 | P0 | Time/size/combined retention | M1 basic → M2 | Slice | G0, G2, G4 | Canonical command v4 replicates complete record-count, compact-JSON-byte, and inclusive-age policies plus explicit idle maintenance without changing v1/v2/v3 bytes. Deterministic core/tablet tests cover exact boundaries, combined oldest-first deletion, oversized-record rollback, dedupe reclamation, monotonic time, digest convergence, and native snapshot restore. Direct and authenticated regional routes plus Go/Java/Python clients expose configure/maintain/observe; real three-runtime checkpoint/reopen and the regional Python SDK container campaign preserve the retained base and policy through recovery. Pending: multi-partition routing, automatic maintenance scheduling, keyed compaction/tombstones, object-tier retention, namespace/legal-hold governance, scale tests, and protected-branch evidence for this increment |
| STREAM-003 | P0 | Consumer groups, offsets, lag, reset/replay | M2 | Slice | G0, G2, G3, G4, G5 | Canonical command v3 replicates a partition-0 next offset, explicit reset, caller-generation owner fencing, typed committed rejection, exact retry, lag and checkpoint replay; deterministic history/digest tests, real three-runtime HTTP commit/reset/fence/reopen, and the container failover/all-voter `SIGKILL` campaign prove recovery while preserving v1/v2. Command v4 retention preserves stale checkpoints, marks `checkpoint_out_of_range`, clamps lag to retained data, and requires an explicit fenced reset before replay. A fully qualified authenticated v1 route and separate Go/Java/Python regional clients add leader discovery, generation/tablet fences, same-key rediscovery, and linearizable reads; Python runs against the post-failover real cluster. Pending: join/heartbeat/assignment/revoke/rebalance, multi-partition ownership, transactions, authorization/audit specificity, generated coordinated-session contracts, scale/fairness, and production fault matrix |
