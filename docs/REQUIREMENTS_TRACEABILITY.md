...
| STREAM-003 | P0 | Consumer groups, offsets, lag, reset/replay | M2 | Slice | G0, G2, G3, G4, G5 | Canonical command v3 replicates one shard's next offset, explicit reset, caller-generation owner fencing, typed committed rejection, exact retry, lag and checkpoint replay; deterministic history/digest tests, real three-runtime HTTP commit/reset/fence/reopen, and the container failover/all-voter `SIGKILL` campaign prove recovery while preserving v1/v2. Command v4 retention preserves stale checkpoints, marks `checkpoint_out_of_range`, clamps lag to retained data, and requires an explicit fenced reset before replay. Authenticated v1 routes and Go/Java/Python clients add leader discovery, logical shard identities, generation/tablet fences, same-key rediscovery, and linearizable reads; Python checkpoints shards 0, 1, and 2 against the post-failover real cluster. Pending: join/heartbeat/assignment/revoke/rebalance, one coordinated group owning several partitions, transactions, authorization/audit specificity, generated coordinated-session contracts, scale/fairness, and production fault matrix |
...
| STREAM-006 | P0 | Batching and required compression paths | M2 | Slice | G2, G4, G6 | Canonical command v2 carries bounded atomic batches through `none`, gzip, LZ4 frame, Snappy framed, and Zstd frame paths; strict unit/golden/malformed/bomb tests, real three-runtime HTTP commit/retry/rebuild, and a Python-generated gzip container campaign prove the replicated per-shard slice while preserving v1 bytes/digests. Regional responses externalize the target logical shard. Pending: stable bidirectional Produce/SDK surface, producer auto-batching and negotiation, cross-shard batch planning, non-atomic partial results, fuzz corpus, and matched compression benchmarks |
...
| ID | Pri | Capability shorthand | Milestone | Status | Dependency gates | Verification evidence placeholder |
|---|---:|---|---|---|---|---|
| DX-001 | P0 | Official Go, Java, and Python SDKs | M1 one SDK → M2 | Slice | G0, G1, G4, G10 | Go/Java/Python standalone tests and exact-source restart quickstarts remain green. All three share regional Stream, complete Queue, complete Cache, and complete implemented Event Bus discovery/auth/fence/idempotent-retry/read-barrier contracts with unit tests and Pages examples. Stream clients additionally expose identical versioned UTF-8 shard selection and generation-pinned keyed append; Python executes three Stream shards and all four profiles after real leader loss. Pending: generated regional response types, coordinated streaming sessions, package publication, and the complete native contract/version matrix |
| DX-002 | P0 | Generated guarantee-aware API docs | M1 → M2 | Slice | G0, G1, G10 | Hand-authored guarantee/error guidance, exact executable standalone quickstarts, and exact compilable regional Stream/Queue/Cache/Event Bus Go/Java/Python sources are built into the docs-only Pages artifact; ADR-0017–0020 and all four regional SDK guides own route/retry/non-claim semantics. Pending: generated API reference and full doc lint |
