...
worker described by ADR-0030. Neither path claims an arbitrary external
business side effect. The executable gates
...
| BUS-003 | P0 | Pull, push, webhook, queue, stream targets | M4 | Slice | G0, G4, G5, G6 | Target-shaped records and a fenced acquire/ack/fail/reject pull-worker protocol are replicated, authenticated, exposed by regional Go/Java/Python clients, queried, recovered, and target-isolated. A built-in regional leader now executes signed HTTP/webhook targets and proves 503/204 retry plus full-voter reopen; pending: long-poll/push/Queue/Stream and unsigned/custom target executors |
| BUS-004 | P0 | Per-target retry, timeout, rate, DLQ | M4 | Slice | G0, G2, G4, G5 | Captured per-subscription timeout/max-in-flight/retry policy, deterministic backoff/jitter, pure lease deadline, bounded automatic timeout maintenance, attempt exhaustion, terminal rejection, dead-letter state, and fencing pass. The signed worker classifies 2xx/429/5xx/other status and network outcomes, caps I/O by the lease, and awaits exact acquisition/settlement proposals. Real-process 503/204 delivery converges two immutable attempts and survives all-voter reopen. Pending: rate limiting, redrive/retention, broader executor crash points, and external audit |
| BUS-005 | P0 | CloudEvents 1.0 over HTTP | M1 envelope → M4 | Slice | G0, G1, G6 | Signed delivery emits the supported envelope as an exact JSON body with CloudEvents 1.0 binary-mode `ce-specversion`, ID, source, type, optional subject, content type, and trace context. Unit and real-receiver round trips pass; pending: official broad conformance corpus, structured mode, extension registry, and non-JSON data |
...
| ID | Pri | Capability shorthand | Milestone | Status | Dependency gates | Verification evidence placeholder |
|---|---:|---|---|---|---|---|
| DX-001 | P0 | Official Go, Java, and Python SDKs | M1 one SDK → M2 | Slice | G0, G1, G4, G10 | Go/Java/Python standalone tests and exact-source restart quickstarts remain green. All three share regional Stream, complete Queue, complete Cache, and implemented Event Bus discovery/auth/fence/retry/read-barrier contracts. Signed target constructors and constant-time exact-body webhook verifiers share the same canonical vector, timestamp window, and replay identity. Stream clients additionally expose identical UTF-8 shard selection, generation-pinned keyed append, atomic caller-framed batches, session lifecycle, low-level per-shard claim/fenced fetch, and a resource-generation-pinned claim–revalidate helper; Python executes three Stream shards and all four profiles after real leader loss. Pending: generated regional response types, persistent native streaming/cooperative revoke, package publication, and the complete native contract/version matrix |
| DX-002 | P0 | Generated guarantee-aware API docs | M1 → M2 | Slice | G0, G1, G10 | Hand-authored guarantee/error guidance, exact executable standalone quickstarts, and exact compilable regional Stream/Queue/Cache/Event Bus Go/Java/Python sources are built into the docs-only Pages artifact. ADR-0030 and the signed-delivery guide add key setup, target construction, exact request/signature contract, three-language receiver verification, replay duties, response behavior, and security/non-claims. Pending: generated API reference and full doc lint |
...
| DX-004 | P0 | Test containers and ephemeral namespaces | M1 → M2 | Slice | G1, G5, G10 | A unique three-node Compose project uses independent volumes, dynamic loopback ports, and mounted policy; it proves Go recovery plus simultaneous four-profile failover/catch-up/all-node recovery. Post-failover Python runs Stream, Queue, Cache, and Event Bus lifecycles. A separate real three-process campaign now binds a real receiver, retries a signed webhook from 503 to 204, converges the two-attempt Ack on every voter, and verifies it after all voters reopen their own storage. Pending: containerized external-target proof, parallel campaigns, Go/Java live regional execution, broader isolation, and injected disk/network matrices |
