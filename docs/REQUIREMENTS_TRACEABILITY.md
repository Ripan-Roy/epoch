...
Last synchronized with PRD version 0.3 on 20 August 2026.
...
state transition; the regional current leader now schedules due retention,
while product-wide retention governance remains open. Regional Streams now materialize several independent
...
| CACHE-002 | P0 | Key/default TTL and expiry events | M1 → M2 | Slice | G0, G1, G2, G4 | Deterministic key/default TTL, pure passive reads, committed bounded maintenance, time rollback, failover, replay, three-language TTL/maintain methods, current-leader regional expiry, and volatile-policy eligibility coupling are tested. The Python campaign observes TTL reclamation without a maintenance call after Cache leader loss and through recovery; pending: expiry events, scale/SLO evidence, and broader fault matrix |
| CACHE-003 | P0 | Eviction policy family | M1 prototype → M2 | Slice | G0, G4, G5 | Replicated entry-count admission implements no-eviction plus deterministic all-key/volatile LRU, LFU, random, and TTL policies. Committed access, canonical tie breaking, no-eligible-victim rollback, snapshot/replay equivalence, config materialization, and real post-failover LRU eviction/reopen are tested; pending: byte-pressure accounting, memory benchmarks, and production scale/SLO evidence |
...
| CACHE-005 | P0 | Pipeline, multiplex, batch, pool guidance | M1 → M2 | Slice | G1, G4 | Go/Java/Python expose `AtomicBatch` over the bounded ordered one-request/one-proposal same-shard transaction and document long-lived transport reuse plus concurrency bounds. Unit and real failover/reopen tests cover correlated atomic results; pending: partial pipelines, native multiplexing, automatic coalescing, and ordering/throughput SLO evidence |
...
| STREAM-002 | P0 | Time/size/combined retention | M1 basic → M2 | Slice | G0, G2, G4 | Canonical command v4 replicates complete record-count, compact-JSON-byte, and inclusive-age policies without changing v1/v2/v3 bytes. Deterministic core/tablet tests cover exact boundaries, combined oldest-first deletion, oversized-record rollback, dedupe reclamation, monotonic time, digest convergence, native snapshot restore, and earliest-deadline selection. Direct/regional routes and Go/Java/Python clients retain explicit configure/maintain/observe. The regional current leader now proposes due idle maintenance automatically at the exact record deadline, and the Python three-node campaign proves post-leader-loss expiry, catch-up, and all-voter reopen without a maintenance call. Pending: keyed compaction/tombstones, object-tier retention, namespace/legal-hold governance, timer scale/SLO evidence, and broader fault matrix |
| STREAM-003 | P0 | Consumer groups, offsets, lag, reset/replay | M2 | Slice | G0, G2, G3, G4, G5 | Canonical v3 commands replicate each shard's next offset, explicit reset, lag/replay, and owner fencing. V5 shard-zero sessions add durable membership, expiry, monotonic generation, and deterministic assignment. V6 claims now preserve each assigned shard's offset while installing a bounded monotonic session fence; exact-member/generation fetch and commit reject stale owners. Go/Java/Python pin resource generation, pre-plan at most 4,096 bridges, claim every shard with deterministic keys, and revalidate the coordinator. Snapshot v3, legacy restore, deterministic/real-voter tests, authenticated routes, and the Python three-shard failover/catch-up/reopen campaign passed in PR #68 and exact-main CI `31726157672`; Pages run `31726157684` published the executable guides. Pending: cooperative revoke acknowledgement, atomic assignment-plus-offset/output transactions, member-bound authorization/audit, persistent streaming transport, generated response types, scale/fairness, and production fault matrix |
...
| BUS-003 | P0 | Pull, push, webhook, queue, stream targets | M4 | Slice | G0, G4, G5, G6 | Target-shaped records and a fenced acquire/ack/fail/reject pull-worker protocol are replicated, authenticated, exposed by regional Go/Java/Python clients, queried, recovered, and target-isolated. PR #74 and exact-main CI `32365193683` prove signed HTTP/webhook execution. The current feature adds source-leader Queue/Stream execution with immutable generation/shard/tablet/epoch binding, shared keyed Stream routing, different-group-leader forwarding, stable target idempotency, committed-target-before-source-Ack ordering, and real all-voter reopen without duplicate insertion; protected PR evidence is pending. Remaining: long-poll/push and unsigned/custom target executors |
| BUS-004 | P0 | Per-target retry, timeout, rate, DLQ | M4 | Slice | G0, G2, G4, G5 | Captured per-subscription timeout/max-in-flight/retry policy, deterministic backoff/jitter, pure lease deadline, bounded automatic timeout maintenance, attempt exhaustion, terminal rejection, dead-letter state, and fencing pass. Signed execution classifies HTTP/network outcomes and Queue/Stream execution carries the same policy through bound-target unavailability; both await exact acquisition, target, and settlement proposals. Real-process tests cover signed 503/204 retry plus Queue/Stream convergence and reopen. Pending: protected evidence for the current slice, rate limiting, redrive/retention, broader executor crash points, and external audit |
| BUS-005 | P0 | CloudEvents 1.0 over HTTP | M1 envelope → M4 | Slice | G0, G1, G6 | Signed delivery emits the supported envelope as an exact JSON body with CloudEvents 1.0 binary-mode `ce-specversion`, ID, source, type, optional subject, content type, and trace context. Unit, real-receiver, protected CI, and published Pages round trips pass; pending: official broad conformance corpus, structured mode, extension registry, and non-JSON data |
...
| ID | Pri | Capability shorthand | Milestone | Status | Dependency gates | Verification evidence placeholder |
|---|---:|---|---|---|---|---|
| DX-001 | P0 | Official Go, Java, and Python SDKs | M1 one SDK → M2 | Slice | G0, G1, G4, G10 | Go/Java/Python standalone tests and exact-source restart quickstarts remain green. All three share regional Stream, complete Queue, Cache committed access/atomic batch, and implemented Event Bus discovery/auth/fence/retry/read-barrier contracts. Signed target constructors and constant-time exact-body webhook verifiers share the same canonical vector, timestamp window, and replay identity. Stream clients additionally expose identical UTF-8 shard selection, generation-pinned keyed append, atomic caller-framed batches, session lifecycle, low-level per-shard claim/fenced fetch, and a resource-generation-pinned claim–revalidate helper; Python executes three Stream shards and all four profiles after real leader loss. Exact-main protection for the current Cache addition remains pending. Pending: generated regional response types, persistent native streaming/cooperative revoke, package publication, and the complete native contract/version matrix |
| DX-002 | P0 | Generated guarantee-aware API docs | M1 → M2 | Slice | G0, G1, G10 | Hand-authored guarantee/error guidance, exact executable standalone quickstarts, and exact compilable regional Stream/Queue/Cache/Event Bus Go/Java/Python sources are built into the docs-only Pages artifact. ADR-0030 covers signed receivers; ADR-0031 and PR #77 cover Queue/Stream target execution; ADR-0032 plus the current Cache guide/examples cover configuration, committed access, deterministic eviction, atomic batches, recovery, and non-claims. Main-only publication of the Cache update remains pending. Pending: generated API reference and full doc lint |
