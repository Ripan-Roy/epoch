...
| STREAM-003 | P0 | Consumer groups, offsets, lag, reset/replay | M2 | Slice | G0, G2, G3, G4, G5 | Canonical command v3 replicates a partition-0 next offset, explicit reset, caller-generation owner fencing, typed committed rejection, exact retry, lag and checkpoint replay; deterministic history/digest tests, real three-runtime HTTP commit/reset/fence/reopen, and the container failover/all-voter `SIGKILL` campaign prove recovery while preserving v1/v2. A fully qualified authenticated v1 route and separate Go/Java/Python regional clients now add leader discovery, generation/tablet fences, same-key rediscovery, and linearizable reads; Python runs against the post-failover real cluster. Pending: join/heartbeat/assignment/revoke/rebalance, multi-partition ownership, retention interaction, transactions, authorization/audit specificity, generated coordinated-session contracts, scale/fairness, and production fault matrix |
...
| ID | Pri | Capability shorthand | Milestone | Status | Dependency gates | Verification evidence placeholder |
|---|---:|---|---|---|---|---|
| QUEUE-001 | P0 | Competing consumers and delivery transitions | M1 prototype → M2 | Slice | G0, G2, G4 | Typed real-runtime enqueue/acquire/Ack and committed rejection; container leader loss, conservative redelivery, convergence, EPRS/all-node restart; pending: concurrent saturation/fairness corpus |
| QUEUE-002 | P0 | Renewable visibility/acquisition lease | M1 prototype → M2 | Slice | G0, G2, G3, G4 | Exact renewed-token HTTP replay, consumer epoch, deadline, active-leader SIGKILL, old-term-token rejection, new-term redelivery, all-voter recovery; pending: exhaustive partition/crash-point matrix |
...
| QUEUE-004 | P0 | Delayed and scheduled messages | M1 prototype → M2 | Slice | G0, G2, G4 | Deterministic committed-order time normalization (including descending failover assignments), TTL/max-age tests, real container scheduled ineligibility/promotion, and explicit committed maintenance; pending: automatic timer proposal and precision load report |
| QUEUE-005 | P0 | Retry/backoff/jitter/attempt-age policy | M1 prototype → M2 | Slice | G0, G2, G4 | Deterministic non-zero jitter/terminal corpus plus typed release/Nack/maintain real-runtime flow and container visibility-timeout retry after failover; pending: broader policy and fault corpus |
| QUEUE-006 | P0 | Provenance-rich DLQ and redrive | M1 prototype → M2 | Slice | G2, G4, G5 | Typed Reject/redrive and immutable stale-capable history APIs converge and survive all-node SIGKILL; stale-history fencing unit proof; pending: authorization and external audit export |
...
| ID | Pri | Capability shorthand | Milestone | Status | Dependency gates | Verification evidence placeholder |
|---|---:|---|---|---|---|---|
| DX-001 | P0 | Official Go, Java, and Python SDKs | M1 one SDK → M2 | Slice | G0, G1, G4, G10 | Go/Java/Python standalone HTTP unit tests and independent exact-source crash/restart quickstarts remain green. All three now also implement the same regional Stream v1 discovery/auth/fence/idempotent-retry/read-barrier contract with unit tests and checked-in compilable Pages examples; Python executes it after real leader loss. Go generated bindings remain covered. Pending: generated regional response types, coordinated streaming sessions, package publication, and the complete native contract/version matrix |
| DX-002 | P0 | Generated guarantee-aware API docs | M1 → M2 | Slice | G0, G1, G10 | Hand-authored guarantee/error guidance, exact executable standalone quickstarts, and exact compilable regional Go/Java/Python sources are built into the docs-only Pages artifact; ADR-0017 and the regional SDK guide own route/retry/non-claim semantics. Pending: generated API reference and full doc lint |
...
| DX-004 | P0 | Test containers and ephemeral namespaces | M1 → M2 | Slice | G1, G5, G10 | A unique three-node Compose project uses independent ephemeral volumes, dynamic loopback ports, and mounted bootstrap policy; it builds a real Go control binary and proves authenticated Go `SIGKILL`/metadata reopen/exact replay plus service-authenticated Go-to-Rust apply/BFF and simultaneous four-profile failover/catch-up/all-node recovery. The post-failover Python regional SDK performs append, exact retry, linearizable fetch, checkpoint, and lag before all-voter restart. The campaign captures scoped failure evidence and cleans only its own resources; pending: parallel campaigns, all-language live regional execution, broader lifecycle/isolation, and injected disk/network matrices |
