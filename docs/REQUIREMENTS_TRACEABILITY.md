...
| DX-002 | P0 | Generated guarantee-aware API docs | M1 → M2 | Slice | G0, G1, G10 | Hand-authored guarantee/error guidance, exact executable standalone quickstarts, and exact compilable regional Stream/Queue/Cache/Event Bus Go/Java/Python sources are built into the docs-only Pages artifact. ADR-0030 and the signed-delivery guide add key setup, target construction, exact request/signature contract, three-language receiver verification, replay duties, response behavior, and security/non-claims. Main-only Pages `32365193694` published and live-bundle verification confirmed that content. Pending: generated API reference and full doc lint |
...
| DX-004 | P0 | Test containers and ephemeral namespaces | M1 → M2 | Slice | G1, G5, G10 | A unique three-node Compose project uses independent volumes, dynamic loopback ports, and mounted policy; it proves Go recovery plus simultaneous four-profile failover/catch-up/all-node recovery. Post-failover Python runs Stream, Queue, Cache, and Event Bus lifecycles. PR #74 and exact-main CI `32365193683` also bind a real receiver, retry a signed webhook from 503 to 204, converge the two-attempt Ack on every voter, and verify it after all voters reopen their own storage. Pending: containerized external-target proof, parallel campaigns, Go/Java live regional execution, broader isolation, and injected disk/network matrices |
