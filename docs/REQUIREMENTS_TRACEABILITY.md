...
G2 remains open because product-wide derived-index rebuild, managed backup and
restore scheduling, external tier providers, and general production replica
recovery are not implemented. Stream logical time/size/combined retention now
...
| BUS-009 | P1 | Schema validation integration | M4 | Slice | G5, G7 | Bounded Avro/JSON Schema/Protobuf definitions, monotonic revisions, structural compatibility, exact references, producer/broker policies, payload rejection, snapshot semantic revalidation, and recovery pass locally. Official format-compiler conformance and schema authorization remain open. |
...
| MGD-014 | P0 | CLI, core SDKs, emulator, operator | M1 core → M2 | Slice | G1, G5, G10 | Go, Java, and Python regional SDKs cover all four native profiles; the generated-contract Go CLI provides strict apply/get/list/delete, OCC, idempotency, and HTTP+gRPC diagnostics; and the leader-elected Go operator reconciles a fixed-three-voter StatefulSet, durable control owner, Services/PVCs, credentials, policy, placement identity, security, and observed status. Fake-Kubernetes race tests, manifest validation, source builds, and the real runtime campaigns pass locally. Pending: published artifacts, TLS identity, live-cluster lifecycle/upgrade/restore evidence, dynamic membership, and a complete emulator command surface |
...
| INT-005 | P1 | Source/target-aware connector checkpoints | M4 | Slice | G2, G7 | Managed targets commit checkpoint-before-source-settlement; generic HTTP sources commit every applied/error-routed record before advancing the exact opaque cursor. Stable source/batch/index proposal identities make crash-before-checkpoint replay duplicate-safe, and a real three-process failover/reopen campaign proves checkpoint convergence. Pending: certified CDC/Kafka/storage cursors and broader crash-point histories |
| INT-006 | P1 | Record errors, partial batch, replay/backfill | M4 | Slice | G4, G7 | Replicated connector state records per-record applied/error-routed outcomes, partial-result metadata, explicit replay intent, and bounded checkpoint history. HTTP source validation rejects malformed/duplicate/oversized batches before commit and isolates a failing connector so later connectors continue. Pending: mixed-result certification for every initial adapter and managed backfill orchestration |
| INT-007 | P1 | Rotatable references and connector egress policy | M4 | Slice | G5, G7 | Source and target workers resolve bounded secret references without placing values in replicated state, enforce endpoint allowlists plus public-address DNS validation/pinning, and disable redirects and ambient proxies. Pending: external secret-manager hot rotation, private managed egress identities, revocation drills, and an independent abuse report |
| INT-008 | P1 | Initial storage, CDC, Kafka, HTTP connectors | M4 | Slice | G5, G6, G7, G8 | Generic HTTP/CloudEvents source polling and managed HTTP target execution are automatic, leader-owned, checkpointed, observable, and recovery-tested. S3-compatible/Azure/GCS storage, PostgreSQL/MySQL CDC, and Kafka adapters remain pending with per-adapter conformance, recovery, security, and load certification |
...
| DX-004 | P0 | Test containers and ephemeral namespaces | M1 → M2 | Slice | G1, G5, G10 | A unique three-node Compose project uses independent volumes, dynamic loopback ports, mounted policy, Go recovery, simultaneous four-profile failover/catch-up/all-node recovery, and post-failover Python lifecycles. Alpha.9 protected evidence includes signed/Epoch/managed delivery and integration state reopen; alpha.10 adds real-process HTTP-source cursor/checkpoint recovery plus operator manifest validation. Pending: live-Kubernetes recovery, parallel campaigns, Go/Java live regional execution, broader isolation, and injected disk/network matrices |
