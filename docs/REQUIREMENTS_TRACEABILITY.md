...
| ID | Pri | Capability shorthand | Milestone | Status | Dependency gates | Verification evidence placeholder |
|---|---:|---|---|---|---|---|
| DX-001 | P0 | Official Go, Java, and Python SDKs | M1 one SDK → M2 | Slice | G0, G1, G4, G10 | Go/Java/Python standalone tests and exact-source restart quickstarts remain green. All three share regional Stream, complete Queue, and complete implemented Cache discovery/auth/fence/idempotent-retry/read-barrier contracts with unit tests and Pages examples; Python executes all three profiles after real leader loss. Pending: generated regional response types, coordinated streaming sessions, Event-Bus regional clients, package publication, and the complete native contract/version matrix |
| DX-002 | P0 | Generated guarantee-aware API docs | M1 → M2 | Slice | G0, G1, G10 | Hand-authored guarantee/error guidance, exact executable standalone quickstarts, and exact compilable regional Stream/Queue/Cache Go/Java/Python sources are built into the docs-only Pages artifact; ADR-0017–0019 and all three regional SDK guides own route/retry/non-claim semantics. Pending: generated API reference and full doc lint |
