...
| DX-004 | P0 | Test containers and ephemeral namespaces | M1 → M2 | Slice | G1, G5, G10 | A unique three-node Compose project uses independent volumes, dynamic loopback ports, and mounted policy; it proves Go recovery plus simultaneous four-profile failover/catch-up/all-node recovery. Post-failover Python runs Stream, Queue, complete Cache, and Event Bus lifecycles. Protected alpha.8 campaigns retry a signed receiver, deliver to Queue/keyed Stream without duplicates, exercise complete Cache and Stream state services, verify Queue sessions/defer/correlation/capacity/crash-safe DLQ forwarding, and reopen every voter. The alpha.9 candidate adds managed structured-CloudEvent delivery, stable idempotency, settlement convergence, and full Event integration state reopen. Pending: protected alpha.9 evidence, parallel campaigns, Go/Java live regional execution, broader isolation, and injected disk/network matrices |
