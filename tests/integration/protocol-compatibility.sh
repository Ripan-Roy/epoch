...
assert_equal original "$(redis_cli SET text-key blocked NX GET)" "Redis SET NX GET previous value"
assert_equal original "$(redis_cli SET text-key replacement GET PX 60000)" "Redis atomic SET GET"
assert_equal replacement "$(redis_cli GET text-key)" "Redis SET GET stored value"
