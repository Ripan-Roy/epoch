...
.PHONY: help bootstrap-check generate generate-check format format-check lint audit test test-unit test-consensus-process test-consensus-probe test-stream-tablet test-integration build check ci compose-config compose-up compose-down compose-probe-config compose-probe-up compose-probe-down clean
...
test-stream-tablet: ## Prove typed Stream fixed-voter majority, failover, and SIGKILL replay.
	@bash tests/integration/stream-tablet.sh

test-integration: test-consensus-process test-consensus-probe test-stream-tablet ## Exercise real processes through consensus, the CLI, and Go/Java/Python SDKs.
